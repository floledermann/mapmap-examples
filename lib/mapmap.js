...
dd.geo = {
    ...
    Line: function(coordinates, properties) {
        return {
            type: 'Feature',
            geometry: {
                type: 'LineString',
    ...
    segments: function() {
        return function(key, data, emit) {
            ...
            for (var i=0; i<data.length; i++) {
                cur = data[i];
                if (prev) {
                    emit(key + '-' + i, dd.geo.Line([[prev.lon,prev.lat],[cur.lon,cur.lat]], prev));
                }
                prev = cur;
            }
        }
...
mapmap.prototype.geometry = function(spec, map, reduceOrOptions, options) {

    var reduce = reduceOrOptions;
    
    if (!options && ! typeof reduce == 'function') {
        options = reduce;
        reduce = null;
    }
        ...
        this._promise.geometry.then(function(topo){
            ...
            new_topo.map(function(t) {
                ...
                if (typeof t.index == 'undefined') {
                    self.layers.push(t.name, t.geometry);
                }
                else {
                    self.layers.insert(t.index, t.name, t.geometry);
                }
            });
            self.draw();
    ...
    if (spec.length) {
        // Array case
        var new_topo = dd.mapreduce(spec, map, reduce);
        self.layers.push(options.layername || ('layer-' + layernum++), new_topo.values());
        ...
        if (geom.type && geom.type == 'Topology') {
            ...
            keys.map(function(k) {
                if (geom.objects[k]) {
                    ...
                    self.layers.push(k, objs);
                    // TODO: support functions for map as well as strings
                    if (map) {
                        for (var i=0; i<objs.length; i++) {
                            var obj = objs[i];
                            if (obj.properties && obj.properties[map]) {
                                objs[i].properties.__key__ = obj.properties[map];
                            }
                        }
                    }
                }
            ...
            if (geom.features) {
                self.layers.push(options.layernames, geom.features);
            }
            else {
                self.layers.push(options.layernames, [geom]);
            }
        }
        // set up projection first to avoid reprojecting geometry
        if (!self.selected_extent) {
...
// TODO
// implement attr() to assign visual attributes, e.g. color, line width etc. from metadata scales
mapmap.prototype.attr = mapmap.prototype.symbolize;
